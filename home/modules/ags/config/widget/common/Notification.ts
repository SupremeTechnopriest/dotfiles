...
import { setupCursorHover } from '@/utils/cursor'
import { MaterialIcon } from './MaterialIcon'
import { CircularProgress } from './CircularProgress'

import type { Time } from '@/types/@girs/glib-2.0/glib-2.0.cjs'
...
const getFriendlyNotifTimeString = (timeObject: Time) => {
  ...
  if (messageTime.compare(oneMinuteAgo!) > 0) return 'Now'
    ...
    return messageTime.format(options.time.format.time.value)
  else if (
    messageTime.get_day_of_year() ==
    GLib.DateTime.new_now_local().get_day_of_year() - 1
  )
    return 'Yesterday'
  else return messageTime.format(options.time.format.date.value)
}
...
const NotificationIcon = (notifObject) => {
  ...
  if (Utils.lookUpIcon(notifObject.appEntry)) icon = notifObject.appEntry
  ...
  return Widget.Box({
    ...
    children: [
      icon != 'NO_ICON'
        ...
        : MaterialIcon(
            `${notifObject.urgency == 'critical' ? 'release_alert' : guessMessageType(notifObject.summary.toLowerCase())}`,
            'lg',
            {
              hexpand: true
            }
...
export const Notification = ({
  ...
  const popupTimeout =
    notifObject.timeout || (notifObject.urgency == 'critical' ? 8000 : 3000)
  ...
  const destroyWithAnims = () => {
    ...
    Utils.timeout(options.transition.value * 2, () => {
      command()
      if (wholeThing) {
        wholeThing.destroy()
      }
        ...
        .on('button-press-event', () => {
          ...
          Utils.timeout(800, () => {
            if (wholeThing?.attribute.held) {
              ...
              Utils.timeout(
                3000,
                () => (notifTextSummary.label = notifObject.summary)
              )
            }
          })
        })
  ...
  const notifExpandButton = Button({
    ...
    child: MaterialIcon('expand_more', 'md', {
      vpack: 'center'
    }),
    setup: setupCursorHover
  })
  ...
  const leftAnim1 = `transition: ${options.transition.value}ms cubic-bezier(0.05, 0.7, 0.1, 1);
                       margin-left: -${Number(maxOffset + endMargin)}rem;
                       margin-right: ${Number(maxOffset + endMargin)}rem;
                       opacity: 0;`

  const rightAnim1 = `transition: ${options.transition.value}ms cubic-bezier(0.05, 0.7, 0.1, 1);
                        margin-left:   ${Number(maxOffset + endMargin)}rem;
                        margin-right: -${Number(maxOffset + endMargin)}rem;
                        opacity: 0;`

  const middleClickClose = `transition: ${options.transition.value}ms cubic-bezier(0.85, 0, 0.15, 1);
                              margin-left:   ${Number(maxOffset + endMargin)}rem;
                              margin-right: -${Number(maxOffset + endMargin)}rem;
                              opacity: 0;`
      ...
      self
        .hook(
          ...
          (self) => {
            ...
            if (
              Math.abs(offset_x) > MOVE_THRESHOLD ||
              Math.abs(offset_y) > MOVE_THRESHOLD
            )
              wholeThing.attribute.held = false
